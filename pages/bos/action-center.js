...
export default function BOSActionCenter() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "manager")
      return actions.filter((a) => a.status === "manager_approved");
    if (filter === "board")
      return actions.filter((a) => a.status === "board_approved");
    if (filter === "dispatch") return actions.filter((a) => a.dispatched);
    if (filter === "clarification")
      return actions.filter((a) => a.status === "needs_clarification");
    return actions;
  }, [actions, filter]);
    ...
    <main className="min-h-screen bg-[#020617] text-white">
        ...
        <div className="mx-auto max-w-7xl px-6 py-7 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-white/60">
              Lifecycle visibility from Ava intake through board approval and
              dispatch.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-6">
        <div className="flex gap-3 flex-wrap">
          {["all", "intake", "manager", "board", "dispatch", "clarification"].map(
            (f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
          ...
          <div key={step.key} className="relative">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p
                  className={
                    step.complete ? "text-white font-medium" : "text-white/40"
                  }
                >
                  {step.label}
                </p>
