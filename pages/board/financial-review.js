  ...
  const highRiskAccounts = useMemo(
    () =>
      ownerBalances.filter((item) => {
        const level = String(
          item.delinquency_level || item.account_health || item.payment_status || ""
        ).toLowerCase();
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="rounded-3xl border border-amber-400/20 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-2xl">
          ...
          <p className="mt-4 max-w-3xl text-slate-300">
            Review owner financial requests, delinquency visibility, payment concerns,
            statement requests, and financial items requiring board awareness.
          </p>
        </div>

        <div className="mt-8 grid gap-5 md:grid-cols-4">
          <Metric label="Financial Requests" value={financialSignals.length} />
          <Metric label="Owner / Ava Reports" value={financialAiEvents.length} tone="violet" />
          <Metric label="High Risk Accounts" value={highRiskAccounts.length} tone="red" />
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-violet-400/20 bg-violet-500/10 p-6">
            ...
            <p className="mt-2 text-sm text-violet-100/70">
              Owner financial concerns, Ava intake activity, and reported financial issues.
            </p>
