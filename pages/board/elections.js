...
const electionWorkflow = [
  "Confirm election timeline and statutory notice requirements",
  "Prepare candidate eligibility and nomination review",
  "Coordinate annual meeting packet and mailing schedule",
  "Track ballots, proxies, quorum, and inspector requirements",
  "Record election outcome and archive governance record",
];
...
export default function Elections() {
  ...
  useEffect(() => {
    loadElectionRecords({ showLoading: true });

    const interval = setInterval(() => {
      loadElectionRecords({ showLoading: false });
    }, 30000);

    return () => clearInterval(interval);
  }, []);

  async function loadElectionRecords({ showLoading = false } = {}) {
    try {
      if (showLoading) {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(
          payload.message || "Unable to load election preparation records."
        );
      }

      const electionRecords = (payload.openRecords || []).filter((record) => {
      ...
      setRecords(electionRecords);
    } catch (error) {
      console.error("Unable to load election preparation records:", error);
      setSystemMessage(
        error.message || "Unable to load election preparation records."
      );
    } finally {
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="rounded-3xl border border-amber-400/20 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-2xl">
          <p className="text-sm uppercase tracking-[0.25em] text-amber-300">
            Election Governance • Annual Meetings • Voting Administration
          </p>

          <div className="mt-5 grid gap-8 lg:grid-cols-[1.35fr_0.65fr]">
            <div>
              <h2 className="text-4xl font-semibold leading-tight">
                Election preparation records now flow from the Admin Operations Intake.
              </h2>

              <p className="mt-5 max-w-3xl text-lg leading-relaxed text-slate-300">
                Election items created from the Admin Dashboard now appear here for
                annual meeting planning, governance tracking, voting preparation,
                notice coordination, and board oversight.
              </p>
            </div>

            <div className="rounded-3xl border border-amber-400/30 bg-amber-400/10 p-6">
              <div className="text-sm text-slate-300">
                Active Election Items
              </div>
              ...
              <div className="mt-4 text-slate-300">
                Live records requiring election visibility.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-4">
          {[
            ["Open Election Records", records.length],
          ...
          <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h3 className="mt-3 text-3xl font-semibold">
                Live Election Preparation Records
              </h3>

              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
                These records originate from the Admin Operations Intake and are
                rendered here because their request type is Election Preparation.
              </p>
            </div>
  ...
  Create Election Record
          ...
          <div className="space-y-5">
            {loadingRecords ? (
              <div className="rounded-3xl border border-white/10 bg-slate-900 p-6 text-sm text-slate-400">
                Loading election preparation records...
              </div>
            ) : records.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-white/10 bg-slate-900/70 p-8">
                ...
                <h4 className="mt-4 text-2xl font-semibold">
                  No open election preparation records
                </h4>

                <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
                  Create a new admin operational record with request type Election
                  Preparation and it will appear here automatically.
                </p>
              </div>
            ) : (
              records.map((record) => (
                  ...
                  <div className="grid gap-6 lg:grid-cols-[1.35fr_0.75fr_0.75fr_0.7fr] lg:items-center">
                    <div>
                      ...
                      <p className="mt-3 leading-relaxed text-slate-300">
                        {record.description ||
                          "Election preparation record submitted for review."}
                      </p>
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
            ...
            <h3 className="mt-3 text-2xl font-semibold">
              Annual Meeting Control Path
            </h3>

            <div className="mt-6 space-y-4">
              {electionWorkflow.map((item) => (
                <div
                  key={item}
          ...
          <div className="rounded-3xl border border-emerald-400/20 bg-emerald-500/10 p-6">
            ...
            <h3 className="mt-3 text-2xl font-semibold text-emerald-100">
              Election Intelligence Expansion
            </h3>
        ...
        <div className="mt-10 rounded-3xl border border-amber-400/20 bg-amber-400/10 p-6">
          ...
          <p className="mt-3 text-slate-300">
            Elections now receives live operational records from the centralized
