...
export default function MaintenanceReview() {
  ...
  async function loadMaintenanceActions() {
    try {
      ...
      setActions([]);
      setSystemMessage(error.message || "Unable to load maintenance review items.");
    } finally {
  ...
  async function loadMaintenanceRecords() {
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message || "Unable to load maintenance operational records.");
      }
      ...
      const records = (payload.openRecords || []).filter((record) => {
        ...
        return (
          ...
          (combined.includes("maintenance") ||
            ...
            combined.includes("capital project"))
        );
      });

      setOperationalRecords(records);
    } catch (error) {
      console.error("Unable to load maintenance operational records:", error);
    } finally {
  ...
  const vendorRecords = useMemo(
    () =>
      operationalRecords.filter((record) => {
        ...
        return combined.includes("vendor") || combined.includes("contractor");
      }),
    [operationalRecords]
  );

  const capitalRecords = useMemo(
    () =>
      operationalRecords.filter((record) => {
        const combined = `${record.request_type || ""} ${record.title || ""} ${
          record.description || ""
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-6 py-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
              Maintenance requests, repair activity, vendor coordination,
              inspections, capital items, and board-aware operational review.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="rounded-3xl border border-amber-400/20 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-2xl">
          ...
          <h2 className="mt-3 max-w-5xl text-4xl font-semibold leading-tight">
            Maintenance review now combines BOS work activity with centralized operational records.
          </h2>

          <p className="mt-4 max-w-4xl text-slate-300">
            Repair requests, owner-reported maintenance items, vendor coordination,
            inspections, capital project awareness, and board review items can now
            flow through Admin Operations Intake while preserving live BOS action visibility.
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href={`/admin/operations/new?request_type=${encodeURIComponent(
                ...
                "/board/maintenance-review"
              )}&return_label=${encodeURIComponent("Maintenance Review")}`}
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-4">
          <Metric label="BOS Maintenance" value={maintenanceSignals.length} />
          <Metric label="Ava / Owner Reports" value={maintenanceAiEvents.length} />
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          <OperationalPanel title="Repair / Maintenance Items" items={repairRecords} />
          <OperationalPanel title="Vendor / Contractor Items" items={vendorRecords} />
          <OperationalPanel title="Capital / Inspection Items" items={capitalRecords} />
        </div>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
            ...
            <div className="space-y-4">
              {loadingActions ? (
                <Empty message="Loading maintenance activity..." />
              ) : maintenanceSignals.length === 0 ? (
                <Empty message="No maintenance review items are currently available." />
              ) : (
          ...
          <div className="rounded-3xl border border-violet-400/20 bg-violet-500/10 p-6">
            ...
            <div className="mt-6 space-y-4">
              {maintenanceAiEvents.length === 0 ? (
                <Empty message="No Ava or owner maintenance reports currently found." />
              ) : (
        ...
        <div className="mt-10 rounded-3xl border border-emerald-400/20 bg-emerald-500/10 p-6">
          <h3 className="text-xl font-semibold text-emerald-100">
            Maintenance Review Connected
          </h3>

          <p className="mt-3 text-slate-300">
            This page now preserves BOS maintenance action visibility while adding
            distributed maintenance records from Admin Operations Intake.
          </p>
        </div>
      </section>
    </main>
          ...
          items.slice(0, 5).map((item) => (
            <div key={item.id} className="rounded-2xl border border-white/10 bg-slate-950/60 p-5">
              <h4 className="font-semibold text-white">
                {item.title || "Untitled Maintenance Record"}
              </h4>
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                <span>{item.request_type || "Maintenance Record"}</span>
                <span>•</span>
...
function MaintenanceCard({ item, isOpen, onToggle }) {
  return (
