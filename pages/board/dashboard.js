...
type Status = "Open" | "In Progress" | "Board Decision" | "Escalated" | "Completed";
...
const initialActions: ActionItem[] = [
  ...
  {
    id: 4,
    title: "Gate access issue resolved",
    ...
    dueDate: "2026-04-30",
    boardDecisionNeeded: false,
    recommendedNextStep: "No further action required.",
    notes: "Resident access card was reactivated and confirmed working.",
...
export default function BOSPhaseThreeDashboard() {
  ...
  const stats = useMemo(() => {
    const open = actions.filter((item) => item.status !== "Completed").length;
    const completed = actions.filter((item) => item.status === "Completed").length;
    const escalated = actions.filter((item) => item.status === "Escalated").length;
    const boardDecisions = actions.filter((item) => item.boardDecisionNeeded).length;

    return { open, completed, escalated, boardDecisions };
  }, [actions]);
      ...
      current.map((item) =>
        item.id === id
          ? {
              ...item,
              status,
              boardDecisionNeeded: status === "Board Decision" ? true : item.boardDecisionNeeded,
            }
          : item
      )
    );
          ...
          <div className="relative mx-auto max-w-7xl px-6 py-10 lg:px-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 px-5 py-4 shadow-2xl shadow-emerald-950/30">
                ...
                <p className="mt-1 text-2xl font-semibold text-emerald-300">Board View Active</p>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="space-y-4">
              <Field label="Action title">
                <input
                  ...
                  className="w-full rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-sm text-white outline-none ring-0 placeholder:text-slate-500 focus:border-teal-300/60"
                />
              </Field>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Category">
                  <select
                    ...
                    className="w-full rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-sm text-white outline-none focus:border-teal-300/60"
                  >
              ...
              <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-900/70 p-4 text-sm text-slate-200">
                <input
                  ...
                  onChange={(event) => setBoardDecisionNeeded(event.target.checked)}
              ...
              <Field label="Notes">
                <textarea
                  ...
                  rows={5}
                  className="w-full resize-none rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-sm text-white outline-none placeholder:text-slate-500 focus:border-teal-300/60"
                />
              </Field>
                  ...
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    ...
                    <div className="flex shrink-0 flex-col gap-2">
                      <select
                        ...
                        className="rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white outline-none focus:border-teal-300/60"
                      >
                        <option>Open</option>
