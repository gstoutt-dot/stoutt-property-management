...
export function calculateAssociationBalanceTotals(records = []) {
  ...
  const severeDelinquencyAccounts = records.filter(
    (item) =>
...
export function calculateDelinquencyLevel(
  currentBalance,
  monthlyAssessment
) {
...
export function calculateAccountHealth(currentBalance) {
  const balance = Number(currentBalance || 0);
...
function normalizePaymentStatus(status) {
  ...
  if (
    ["current", "paid", "zero_balance"].includes(value)
  ) {
    return "current";
  }
