...
export async function sendBoardMessageEmailAlert({
  ...
  const safeMessageBody = cleanText(messageBody, "");
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
      ...
      ${
        safeMessageBody
          ? `<div style="margin: 18px 0; padding: 16px; border-left: 4px solid #d97706; background: #f9fafb;">
              <p style="margin: 0; white-space: pre-wrap;">${safeMessageBody}</p>
            </div>`
          : ""
      }
...
export async function sendHomeownerNotificationEmailAlert({
  to,
  ...
  const emailSubject = cleanText(
    subject,
    "New notice from Stoutt Property Management"
  );

  const safeNotificationTitle = cleanText(notificationTitle, "Association Notice");
  const safeMessageBody = cleanText(messageBody, "");

  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
      <h2>New Homeowner Notice</h2>
      <p>Hello ${recipientName},</p>
      <p>Management has sent a homeowner notice regarding:</p>
      <p><strong>${safeNotificationTitle}</strong></p>
      ${
        safeMessageBody
          ? `<div style="margin: 18px 0; padding: 16px; border-left: 4px solid #d97706; background: #f9fafb;">
              <p style="margin: 0; white-space: pre-wrap;">${safeMessageBody}</p>
      ...
      <p>Please log in to SPM to review your homeowner message center.</p>
      <p style="margin-top: 24px;">
        Stoutt Property Management<br />
        Homeowner Communication Center
      </p>
    </div>
  `;
